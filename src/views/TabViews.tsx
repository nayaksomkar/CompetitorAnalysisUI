...
function PageHeader({ title, subtitle, action }: { title: string; subtitle?: string; action?: React.ReactNode }) {
  return (
  <header className="px-4 sm:px-6 pt-4 sm:pt-6 pb-3 border-b border-ink-100  flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
  <div className="min-w-0">
  <h1 className="text-lg sm:text-xl font-semibold text-ink-900  truncate">{title}</h1>
  {subtitle && <p className="text-sm text-ink-500  mt-1 truncate">{subtitle}</p>}
  </div>
