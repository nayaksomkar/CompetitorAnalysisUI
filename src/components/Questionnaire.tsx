...
export function Questionnaire({ onSubmit }: QuestionnaireProps) {
  const [sampleId, setSampleId] = useState<SampleId>('perfume');
    ...
    <div className="min-h-full flex flex-col items-center px-4 py-10 bg-gradient-to-b from-white to-ink-50/40">
      <div className="max-w-3xl w-full">
        ...
        <div className="card p-6 mb-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-ink-500 mb-3">Try a sample analysis</p>
          <div className="grid sm:grid-cols-2 gap-2">
            {sampleList.map((s) => (
              <button
                key={s.id}
                onClick={() => switchSample(s.id)}
                className={`text-left rounded-xl border p-3 transition ${sampleId === s.id ? 'border-ink-900 ring-1 ring-ink-900 bg-ink-50/40' : 'border-ink-200 hover:border-ink-300 bg-white'}`}
              >
          ...
          <p className="text-xs text-ink-400 mt-3">Selecting a sample pre-fills the form so you can edit and submit.</p>
        </div>

        <form
          className="card p-6 space-y-5"
          onSubmit={(e) => { e.preventDefault(); onSubmit(form, sampleId); }}
        >
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label="Pricing">
              <input className="input" value={form.pricing ?? ''} onChange={(e) => update('pricing', e.target.value)} placeholder="$29 / month" />
            </Field>
