...
export function ActionPlanList({
  ...
  <span className={`pill ${p === 'P0' ? 'bg-rose-50  text-rose-700  border border-rose-100 ' : p === 'P1' ? 'bg-amber-50  text-amber-700  border border-amber-100 ' : 'bg-ink-50  text-ink-600  border border-ink-100 '}`}>
  {p === 'P0' ? 'Ship now' : p === 'P1' ? 'Next' : 'Later'}
  </span>
  <span className="text-xs text-ink-500 ">{grouped[p].length} item{grouped[p].length === 1 ? '' : 's'}</span>
  </div>
  <ul className="space-y-2">
  {grouped[p].map((i) => (
  <li key={i.id} className="rounded-xl border border-ink-100  hover:border-ink-200 hover:shadow-soft transition p-3">
  <div className="flex items-start justify-between gap-3">
  <div className="min-w-0">
  <p className="font-medium text-ink-900 ">{i.title}</p>
  <p className="text-sm text-ink-600  mt-1 leading-relaxed">{i.description}</p>
  {i.rationale && (
  <p className="text-xs text-ink-500  mt-1.5"><span className="font-semibold">Why:</span> {i.rationale}</p>
  )}
  </div>
  <div className="shrink-0 flex flex-col items-end gap-1 text-xs text-ink-500 ">
  {i.owner && <span><span className="text-ink-400 ">Owner:</span> <span className="text-ink-700 ">{i.owner}</span></span>}
  <span>Effort: <span className="text-ink-700 ">{i.effort}</span></span>
  <span>Impact: <span className="text-ink-700 ">{i.impact}</span></span>
  <span>Horizon: <span className="text-ink-700 ">{i.horizon}</span></span>
  </div>
  </div>
  </li>
  ))}
...
export function SourcesList({ sources }: { sources: Source[] }) {
  ...
  <h3 className="font-semibold text-ink-900 ">All sources</h3>
  <p className="text-xs text-ink-500  mt-1">Every claim in this analysis links back to one of the items below.</p>
  </div>
  <ul className="divide-y divide-ink-100 ">
  {sources.map((s) => (
  <li key={s.id} className="p-4 hover:bg-ink-50/30">
  <div className="flex items-start justify-between gap-3">
  <div className="min-w-0">
  <p className="font-medium text-ink-900  truncate">{s.title}</p>
  <p className="text-xs text-ink-500  mt-0.5">{s.publisher} · {s.date}</p>
  {s.snippet && <p className="text-sm text-ink-600  mt-2">{s.snippet}</p>}
  </div>
  {s.url && (
  <a href={s.url} target="_blank" rel="noreferrer" className="text-ink-500 hover:text-ink-900 shrink-0">
  <Icon.External />
  </a>
  )}
  </div>
