      ...
      <div className="flex items-center justify-between gap-3 p-4 border-b border-ink-100 flex-wrap">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="text-sm rounded-lg border border-ink-200 pl-8 pr-3 py-1.5 outline-none focus:border-ink-400"
          />
        </div>
      </div>
...
function PricingCard({ tier, billing }: { tier: PricingTier; billing: 'monthly' | 'yearly' }) {
  const price = tier.priceMonthly === 'Custom' ? 'Custom' : `$${tier.priceMonthly}`;
  const cycle = tier.priceMonthly === 'Custom' ? '' : billing === 'yearly' ? '/seat/mo · billed yearly' : '/seat/mo';
