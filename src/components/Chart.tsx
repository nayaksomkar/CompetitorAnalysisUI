...
export function Chart({ data }: { data: ChartData }) {
  ...
  <div className="inline-block bg-white  border border-ink-200  rounded-lg px-3 py-1.5 shadow-sm">
  <p className="text-xs font-semibold text-ink-900 ">{pieSlices[hover].label}</p>
  <p className="text-xs text-ink-500 ">{pieSlices[hover].value} ({pieSlices[hover].pct}%)</p>
  </div>
  </div>
  )}
  </div>
  ...
  <span className="flex-1 text-ink-700  truncate">{slice.label}</span>
  <span className="font-medium text-ink-900  tabular-nums">{Math.round((slice.value / pieTotal) * 100)}%</span>
  </div>
  ))}
  </div>
  </ChartCard>
...
function Legend({ series, hover, setHover }: { series: { name: string; color?: string }[]; hover: number | null; setHover: (v: number | null) => void }) {
  return (
  <div className="mt-3 pt-3 border-t border-ink-100  flex flex-wrap gap-x-4 gap-y-1.5">
  {series.map((s, si) => (
  <div key={si}
